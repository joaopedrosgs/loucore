...
func (cq *CityQuery) QueryQueue() *QueueItemQuery {
...
func (cq *CityQuery) WithQueue(opts ...func(*QueueItemQuery)) *CityQuery {
	query := &QueueItemQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CityQuery) sqlAll(ctx context.Context) ([]*City, error) {
	...
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, city.ForeignKeys...)
	}
	...
	if query := cq.withOwner; query != nil {
		...
		for i := range nodes {
			if fk := nodes[i].user_cities; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_cities" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := cq.withConstructions; query != nil {
		...
		query.Where(predicate.Construction(func(s *sql.Selector) {
			s.Where(sql.InValues(city.ConstructionsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.city_constructions
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "city_constructions" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "city_constructions" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := cq.withQueue; query != nil {
		...
		query.Where(predicate.QueueItem(func(s *sql.Selector) {
			s.Where(sql.InValues(city.QueueColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.city_queue
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "city_queue" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "city_queue" returned %v for node %v`, *fk, n.ID)
			}
