...
type UserQuery struct {
	...
	withConstructions *ConstructionQuery
...
func (uq *UserQuery) QueryQueue() *QueueItemQuery {
	query := &QueueItemQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryConstructions() *ConstructionQuery {
	query := &ConstructionQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withCities:        uq.withCities.Clone(),
		withQueue:         uq.withQueue.Clone(),
		withConstructions: uq.withConstructions.Clone(),
...
func (uq *UserQuery) WithQueue(opts ...func(*QueueItemQuery)) *UserQuery {
	query := &QueueItemQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) WithConstructions(opts ...func(*ConstructionQuery)) *UserQuery {
	query := &ConstructionQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withConstructions = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withCities; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Cities = []*City{}
		}
		query.withFKs = true
		query.Where(predicate.City(func(s *sql.Selector) {
			s.Where(sql.InValues(user.CitiesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_cities
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_cities" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_cities" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Cities = append(node.Edges.Cities, n)
		}
	}

	if query := uq.withQueue; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.QueueItem(func(s *sql.Selector) {
			s.Where(sql.InValues(user.QueueColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_queue
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_queue" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_queue" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Queue = append(node.Edges.Queue, n)
		}
	}

	if query := uq.withConstructions; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.Construction(func(s *sql.Selector) {
			s.Where(sql.InValues(user.ConstructionsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_constructions
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_constructions" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_constructions" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Constructions = append(node.Edges.Constructions, n)
		}
	}
