...
func (uu *UserUpdate) AddQueue(q ...*QueueItem) *UserUpdate {
	ids := make([]int, len(q))
	for i := range q {
		ids[i] = q[i].ID
	}
...
func (uu *UserUpdate) RemoveQueue(q ...*QueueItem) *UserUpdate {
	ids := make([]int, len(q))
	for i := range q {
		ids[i] = q[i].ID
	}
...
// AddQueue adds the queue edges to QueueItem.
func (uuo *UserUpdateOne) AddQueue(q ...*QueueItem) *UserUpdateOne {
	ids := make([]int, len(q))
	for i := range q {
		ids[i] = q[i].ID
	}
...
// RemoveQueue removes queue edges to QueueItem.
func (uuo *UserUpdateOne) RemoveQueue(q ...*QueueItem) *UserUpdateOne {
	ids := make([]int, len(q))
	for i := range q {
		ids[i] = q[i].ID
	}
